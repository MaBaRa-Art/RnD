...
ANGLE_BITS = 21 # angles range from 0..65535
...
class AngleCalibration:
    ...
    def apply_calibration(self, samples):
        ...
        angle_mask = (1 << ANGLE_BITS) - 1 # 21-bit mask
        sign_bit = 1 << (ANGLE_BITS - 1)
        for i, (samp_time, angle) in enumerate(samples):
            bucket = (angle & angle_mask) >> interp_bits
            cal1 = calibration[bucket]
            cal2 = calibration[bucket + 1]
            adj = (angle & interp_mask) * (cal2 - cal1)
            adj = cal1 + ((adj + interp_round) >> interp_bits)
            angle_diff = (adj - angle) & angle_mask
            angle_diff -= (angle_diff & sign_bit) << 1
    ...
    def get_stepper_phase(self):
        mcu_phase_offset, phases = self.tmc_module.get_phase_offset()
        if mcu_phase_offset is None:
            raise self.printer.command_error("Driver phase unknown for %s"
                                             % (self.stepper_name,))
    ...
    def do_calibration_moves(self):
        ...
        if len(cal) != len(times):
            raise self.printer.command_error("Calibration failed - sensor data error")
...
# QUESTE RIGHE DEVONO AVERE SOLO 4 SPAZI DAL MARGINE SINISTRO
    cmd_ANGLE_CALIBRATE_help = "Calibrate the magnetic angle sensor"
    ...
    def cmd_ANGLE_CALIBRATE(self, gcmd):
        ...
        if not fangles or not rangles:
            raise gcmd.error("Calibration failed - no data collected")
            
        merged = { i: fcal.get(i, []) + rcal.get(i, []) for i in range(len(fcal)) }
        ...
        angle_max = 1 << ANGLE_BITS
        anglist = [angles[i] % angle_max for i in range(len(angles))]
        ...
        gcmd.respond_info("Calibration complete. Use SAVE_CONFIG to save to printer.cfg")
...
class Angle:
    def __init__(self, config):
        ...
        config_cmd_name = "config_spi_angle"
        if sensor_type == "mt6835_custom":
            config_cmd_name = "config_mt6835_custom"
            
        mcu.add_config_cmd(
            "%s oid=%d spi_oid=%d spi_angle_type=%s"
            % (config_cmd_name, oid, self.spi.get_oid(), sensor_type))
        mcu.add_config_cmd(
    ...
    def _extract_samples(self, raw_samples):
        ...
        time_shift = 0
        static_delay = 0.
        angle_mask = (1 << ANGLE_BITS) - 1 # Crea il limite a 2.097.151
        sign_bit = 1 << (ANGLE_BITS - 1)# Definisce il bit di segno per i 21-bit
        ...
        for params in raw_samples:
            ...
            for i in range(len(d) // BYTES_PER_SAMPLE):
                ...
                if tcode == TCODE_ERROR:
                    error_count += 1
                    continue
                raw_angle = d_ta[1] | (d_ta[2] << 8) | (d_ta[3] << 16)
                angle_diff = (raw_angle - last_angle) & angle_mask # Usa angle_mask (Riga 505)
                angle_diff -= (angle_diff & sign_bit) << 1 # Usa sign_bit (Riga 506)
